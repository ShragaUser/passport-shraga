...
const IssuerEncoding = (shragaURL, callbackURL, SignInSecret, useEnrichId) =>
...
function authenticate(req, options) {
  ...
  if (requestVerbIsGet(req)) {
    ...
    self.redirect(IssuerEncoding(options.shragaURL, callbackURL, signKey, options.useEnrichId));
